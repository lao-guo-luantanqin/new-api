...
FROM (VALUES
    ('leonardo-seedance-2.0', 'Leonardo 订阅号 Seedance 2.0。文生/图生/首尾帧，480P/720P/1080P，4–15 秒。', 'video,seedance,leonardo,subscription'),
...
-- 2. profile 提示：Leonardo 订阅号暂不支持 933 参考音视频
UPDATE model_ui_param_profiles SET
    option_rules = (
        ...
        FROM (
            ...
            SELECT * FROM jsonb_array_elements('[
                {"param":"reference_videos","disabledWhen":{"modelIncludes":"leonardo-seedance"}},
                {"param":"reference_audios","disabledWhen":{"modelIncludes":"leonardo-seedance"}},
    ...
    hints = (
        ...
        FROM (
            ...
            UNION ALL
            SELECT * FROM jsonb_array_elements('[
                {"text":"Leonardo 订阅号：文生/图生/首尾帧；暂不支持参考视频/音频（933）。时长 4–15 秒。","when":{"modelIncludes":"leonardo-seedance"}}
            ]'::jsonb)
        ) merged(elem)
    ),
    updated_time = EXTRACT(EPOCH FROM NOW())::BIGINT
